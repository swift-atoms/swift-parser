...
extension Prefix.Through where Delimiter: Swift.Collection, Delimiter.Element: Equatable {
    ...
    where Input.SubSequence == Input, Input.Element == Delimiter.Element {
        ...
        public typealias Failure = Prefix.Through<Delimiter>.Error
        public let wrapped: Prefix.Through<Delimiter>

        public init(_ wrapped: Prefix.Through<Delimiter>) { self.wrapped = wrapped }
...
extension Parser::Builder where Input: Swift.Collection, Input.SubSequence == Input {
    public static func buildExpression<D: Swift.Collection>(_ selection: Prefix.Through<D>) -> Prefix.Through<D>.Parser<Input>

...
Output: ~Copyable & ~Escapable
{
    ...
    ) -> Map::Map<Output, NewOutput, Failure>.Parser<Self> {
        .init(
            upstream: self,
            transform: transform,
            failure: { $0 }
    ...
    ) -> Map::Map<Output, NewOutput, Either<Failure, TransformFailure>>.Parser<Self> {
        .init(
            upstream: self,
...
Failure == Never
{
    ...
    ) -> Map::Map<Output, NewOutput, TransformFailure>.Parser<Self> {
        .init(
            upstream: self,
            transform: transform,
            failure: { $0 }

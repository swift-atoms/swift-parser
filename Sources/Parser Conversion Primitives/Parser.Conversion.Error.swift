...
extension Parser.Conversion {
    ...
    /// (``Parser/Conversion/RawValue``, ``Parser/Conversion/Fixed``,
    /// ``Parser/Conversion/Case``) raise a case of this error. Payload-free so it
    /// stays Embedded-friendly.
    public enum Error: Swift.Error, Sendable, Equatable {
        ...
        /// ``Parser/Conversion/Case`` received a root value during `unapply`
        /// whose enum case did not match the one the conversion addresses (its
        /// `extract` closure returned `nil`).
        case absentCase
    }
}

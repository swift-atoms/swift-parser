...
private struct `Pair parser ownership constraints survive module emission` {
    ...
    func `transferring a Pair of owners into a parser prevents its reuse`() throws {
        let diagnostic = try emissionFailure(named: "Consumed Pair Owner.swift")
    ...
    func `a Pair parser with noncopyable stored owners cannot be copied`() throws {
        let diagnostic = try emissionFailure(named: "Noncopyable Pair Copyability.swift")
    ...
    private func emitFixture(named name: String) throws -> (status: Int32, diagnostic: String) {
        ...
        let output = FileManager.default.temporaryDirectory
            .appendingPathComponent("pair-parser-emission-\(UUID().uuidString)")
